...
export default function ResearchPage() {
  ...
  return (
    <div>
      ...
      <div className="rounded-xl bg-white/[0.02] border border-white/[0.06] p-4 mb-4">
        <div className="flex flex-col md:flex-row gap-2">
          ...
          <button
            ...
            className="px-4 py-2 rounded-lg bg-white/[0.06] hover:bg-white/[0.1] disabled:opacity-50 text-white text-sm transition"
          >
            {isSearchingIdeas ? (
              <span className="inline-flex items-center gap-1.5">
          ...
          <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-2">
            {ideas.map((idea) => (
              <a
                ...
                target="_blank"
                rel="noreferrer"
                className="rounded-lg border border-white/[0.08] bg-white/[0.02] p-3 hover:border-brand-500/40 transition"
              >
                <div className="text-xs text-brand-300 mb-1 line-clamp-1">{idea.title}</div>
                <div className="text-[11px] text-gray-400 line-clamp-2">{idea.snippet}</div>
              </a>
            ))}
          </div>
        )}
      ...
      {loadingHistory ? (
        <div className="text-center py-12 text-gray-400 text-sm">Incarc istoricul...</div>
      ) : (
            ...
            >
              <div className="flex items-start justify-between mb-3 gap-3">
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-gray-500 truncate">{hostnameFromUrl(item.url)}</div>
                ...
                <div className="flex gap-1 shrink-0">
                  <button
                    ...
                    className="p-1 text-gray-500 hover:text-brand-300 transition"
                    title="Reanalizeaza"
                  >
              ...
              <div className="mb-3">
                ...
                <p className="text-xs text-gray-400 leading-relaxed">{item.contentStrategy}</p>
              </div>
              ...
              {item.recommendations.length > 0 && (
                <div>
                  <div className="text-[10px] uppercase tracking-wider text-gray-500 mb-1">Recomandari</div>
                  <ul className="space-y-1">
                    {item.recommendations.slice(0, 3).map((rec, index) => (
                      <li key={`${item.id}-rec-${index}`} className="text-xs text-gray-400 leading-relaxed">
                        - {rec}
                      </li>
                    ))}
                  </ul>
