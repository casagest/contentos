...
export default function BusinessProfileForm({
  ...
  const inputClass =
    "w-full bg-muted border border-border rounded-lg px-3 py-2 text-sm text-white placeholder:text-gray-600 focus:outline-none focus:ring-2 focus:ring-brand-500/40";
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-lg text-xs font-medium transition border ${
                    selected
                      ...
                      : "bg-card border-border text-muted-foreground hover:text-white hover:border-border"
                  }`}
                >
          ...
          <div className="space-y-2">
            {COMPLIANCE_OPTIONS.map((option) => {
              ...
              return (
                <label
                  key={option.value}
                  className="flex items-start gap-3 p-3 rounded-lg bg-card border border-border cursor-pointer hover:border-border transition"
                >
                  <input
                    type="checkbox"
                    checked={checked}
                    onChange={() => toggleCompliance(option.value)}
                    className="mt-0.5 rounded border-white/20 bg-muted text-brand-600 focus:ring-brand-500/40"
                  />
                  <div>
